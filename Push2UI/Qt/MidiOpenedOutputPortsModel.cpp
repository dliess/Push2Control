...
QString MidiOpenedOutputPortsModel::getDevName(int idx) const noexcept
{
    ...
    {
        return QString(m_rMidiOpenedPorts.soundDevices.atIdx(idx).second->pDescr->productName.c_str());
    }
    return QString();
}

QUrl MidiOpenedOutputPortsModel::getDevPicturePath(int idx) const noexcept
{
    if(idx < m_rMidiOpenedPorts.soundDevices.size())
    {
        return QUrl(("qrc:///Artwork/Devices/" + m_rMidiOpenedPorts.soundDevices.atIdx(idx).second->pDescr->configPath + "/Thumbnail.png").c_str());
    }
    return QUrl();
}
